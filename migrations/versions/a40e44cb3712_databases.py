...
def upgrade():
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('subname', sa.String(length=120), nullable=True),
    sa.Column('totalclass', sa.Integer(), nullable=True),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_subject_subname'), 'subject', ['subname'], unique=False)
    ...
    op.create_index(op.f('ix_teacher_subject'), 'teacher', ['subject'], unique=False)
    op.create_index(op.f('ix_teacher_username'), 'teacher', ['username'], unique=True)
    op.create_table('attendance',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('subid', sa.String(length=10), nullable=False),
    sa.Column('datemissed', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['id'], ['student.id'], ),
    sa.ForeignKeyConstraint(['subid'], ['subject.id'], ),
    sa.PrimaryKeyConstraint('id', 'subid', 'datemissed')
    )
...
def downgrade():
    ...
    op.drop_index(op.f('ix_teacher_subject'), table_name='teacher')
    op.drop_index(op.f('ix_teacher_email'), table_name='teacher')
    op.drop_table('teacher')
    op.drop_index(op.f('ix_subject_subname'), table_name='subject')
